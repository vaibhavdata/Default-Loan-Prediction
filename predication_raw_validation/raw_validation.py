...
class Raw_Data_validation:
    ...
    def value_from_schema(self):
        try:
            ...
            file =open("Predication_Logs/valueFromSchema.txt","a+")
    ...
    def manaualRegrexCreation(self):
        regex = "['LoanDelinquencyPrediction']+['\_'']+[\d_]+[\d]+\.csv"
        return regex
    def createDiretorGoodBadROwData(self):
        try:
            ...
            file = open("Predication_Logs/GeneralLog.txt", 'a+')
    ...
    def validationFileName(self,regex,LengthOfDateStampInFile,LengthOfTimeStampInFile):
        ...
        try:
            f = open("Predication_Logs/nameValidationLog.txt", 'a+')
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        if len(splitAtDot[2]) == LengthOfTimeStampInFile:
                            shutil.copy("predication_Batch_File/" + filename, "Prediction_Raw_files_validated/Good_Raw")
                            self.logger.log(f,"Valid File name!! File moved to GoodRaw Folder :: %s" % filename)
                        else:
                            shutil.copy("predication_Batch_File/" + filename, "Prediction_Raw_files_validated/Bad_Raw")
    ...
    def validationColumnNumber(self,NumberOfColumn):
        try:
            f = open("Predication_Logs/columnValidationLog.txt", 'a+')
