    ...
    def insertion_GoodRawData_into_collection(self,collection_object):
        ...
        try:
            ...
            for f in only_files:
                ...
                document = [{'loan_id':id,'source':source,'financial_institution':finan,'interest_rate':interest,'unpaid_principal_bal':unpaid,'loan_term':loan_term,'origination_date':origination_date,'first_payment_date':first_pay,'loan_to_value':loan_to_val,'number_of_borrowers':number_of,'debt_to_income_ratio':debt_to,'borrower_credit_score':borrower,'loan_purpose':loan_purp,'insurance_percent':insurance_pre,'co-borrower_credit_score':co_borr,'insurance_type':insurance,'m1':m1,'m2':m2,'m3':m3,'m4':m4,'m5':m5,'m6':m6,'m7':m7,'m8':m8,'m9':m9,'m10':m10,'m11':m11,'m12':m12,'m13':m13} for id,source,finan,interest,unpaid,loan_term,origination_date,first_pay,loan_to_val,number_of,debt_to,borrower,loan_purp,insurance_pre,co_borr,insurance,m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12,m13 in zip(data['loan_id'],data['source'],data['financial_institution'],data['interest_rate'],data['unpaid_principal_bal'],data['loan_term'],data['origination_date'],data['first_payment_date'],data['loan_to_value'],data['number_of_borrowers'],data['debt_to_income_ratio'],data['borrower_credit_score'],data['loan_purpose'],data['insurance_percent'],data['co-borrower_credit_score'],data['insurance_type'],data['m1'],data['m2'],data['m3'],data['m4'],data['m5'],data['m6'],data['m7'],data['m8'],data['m9'],data['m10'],data['m11'],data['m12'],data['m13'])]
